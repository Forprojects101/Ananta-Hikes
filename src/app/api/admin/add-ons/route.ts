...
export async function POST(request: NextRequest) {
  try {
    console.log("🏔️ [POST /admin/add-ons] Request received");
    
    const supabase = getSupabaseServerClient();
    if (!(await ensureAdmin(request, supabase))) {
      console.warn("⚠️ [POST /admin/add-ons] Unauthorized access attempt");
      return NextResponse.json({ message: "Unauthorized" }, { status: 403 });
    }

    const body = await request.json();
    console.log("📥 [POST /admin/add-ons] Request body:", JSON.stringify(body, null, 2));

    const { mountain_id, name, description, price, is_active } = body;

    if (!mountain_id) {
      console.warn("⚠️ [POST /admin/add-ons] Mountain ID is required");
      return NextResponse.json({ message: "Mountain ID is required" }, { status: 400 });
    }

    if (!name || !name.trim()) {
      console.warn("⚠️ [POST /admin/add-ons] Add-on name is required");
    ...
    console.log("💾 [POST /admin/add-ons] Inserting add-on:", JSON.stringify(payload, null, 2));
    ...
    if (error) {
      console.error("❌ [POST /admin/add-ons] Supabase error:", error);
      return NextResponse.json(
        { message: "Failed to create add-on", error: error.message },
        { status: 500 }
      );
    }

    console.log("✅ [POST /admin/add-ons] Add-on created successfully:", data?.[0]);
    return NextResponse.json(data?.[0], { status: 201 });
  } catch (error) {
    console.error("❌ [POST /admin/add-ons] Catch block error:", error);
    return NextResponse.json(
      {
