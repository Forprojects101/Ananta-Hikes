          ...
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="lg:w-1/3">
              <div
                ...
                className="relative group/preview rounded-3xl aspect-[4/3] overflow-hidden bg-gray-50 border-2 border-dashed border-slate-200 transition-all hover:border-primary-300 shadow-inner cursor-pointer"
              >
                {addForm.image_url ? (
                  <>
                  ...
                  <div className="w-full h-full flex flex-col items-center justify-center text-slate-300 gap-2 group-hover/preview:text-primary-400">
                    {isCropping ? (
                      <Loader2 className="animate-spin text-primary-500" size={32} />
                    ) : (
                      <>
                        <div className="p-3 rounded-2xl bg-white shadow-sm group-hover/preview:scale-110 transition-transform">
                          <Camera size={24} />
                        </div>
                        <span className="text-[9px] font-black uppercase tracking-[0.2em] opacity-50 text-center px-4">Click to Upload Media</span>
                      </>
                    )}
                  </div>
                )}
            ...
            <div className="flex-1 space-y-3">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="md:col-span-2">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5 block">Title *</label>
                  <input
                    type="text"
                    value={addForm.title}
                    onChange={(e) => setAddForm({ ...addForm, title: e.target.value })}
                    placeholder="e.g., Summit Success - Team North"
                    className="w-full px-4 py-2.5 bg-slate-50 rounded-xl border-2 border-transparent focus:bg-white focus:border-primary-500 transition-all outline-none font-bold text-sm"
                    disabled={isActionLoading}
                  />
                </div>

                <div className="md:col-span-2">
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5 block">Description</label>
                  <input
                    type="text"
                    ...
                    className="w-full px-4 py-2.5 bg-slate-50 rounded-xl border-2 border-transparent focus:bg-white focus:border-primary-500 transition-all outline-none font-medium text-sm"
                    disabled={isActionLoading}
                  />
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <select
                      ...
                      className="w-full pl-10 pr-4 py-2.5 bg-slate-50 rounded-xl border-2 border-transparent focus:bg-white focus:border-primary-500 transition-all outline-none font-bold text-sm appearance-none cursor-pointer"
                      disabled={isActionLoading}
                    >
                ...
                <div>
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5 block">Date</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-4 py-2.5 bg-slate-50 rounded-xl border-2 border-transparent focus:bg-white focus:border-primary-500 transition-all outline-none font-bold text-sm"
                      disabled={isActionLoading}
                    />
                  </div>
                </div>
                ...
                <div>
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5 block">Group Classification</label>
                  <div className="relative">
                    <UsersIcon className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-300" size={16} />
                    <select
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-slate-100">
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2 cursor-pointer group/toggle" onClick={() => setAddForm({ ...addForm, is_featured: !addForm.is_featured })}>
                    <div className={`w-8 h-5 rounded-full transition-all duration-300 flex items-center px-1 ${addForm.is_featured ? 'bg-amber-400' : 'bg-slate-200'}`}>
                      <div className={`w-3 h-3 rounded-full bg-white transition-all duration-300 ${addForm.is_featured ? 'translate-x-3' : 'translate-x-0'}`} />
                    </div>
                    <span className="text-[9px] font-black uppercase tracking-widest text-slate-400 group-hover/toggle:text-slate-900">Featured</span>
                  </div>
                  <div className="flex items-center gap-2 cursor-pointer group/toggle" onClick={() => setAddForm({ ...addForm, is_active: !addForm.is_active })}>
                    <div className={`w-8 h-5 rounded-full transition-all duration-300 flex items-center px-1 ${addForm.is_active ? 'bg-emerald-500' : 'bg-slate-200'}`}>
                      <div className={`w-3 h-3 rounded-full bg-white transition-all duration-300 ${addForm.is_active ? 'translate-x-3' : 'translate-x-0'}`} />
                    </div>
                    <span className="text-[9px] font-black uppercase tracking-widest text-slate-400 group-hover/toggle:text-slate-900">Visibility</span>
                  </div>
                </div>
                ...
                <div className="flex gap-3 w-full sm:w-auto">
                  <button
                    ...
                    className="flex-1 sm:flex-none px-6 py-3 rounded-xl border-2 border-slate-100 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-slate-600 hover:bg-slate-50 transition-all"
                  >Discard</button>
                  <button
                    type="submit"
                    disabled={isActionLoading}
                    className="flex-1 sm:flex-none px-6 py-3 rounded-xl bg-primary-600 text-white text-[10px] font-black uppercase tracking-widest hover:bg-primary-700 shadow-xl shadow-primary-500/20 active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                  >
                    {isActionLoading ? "Saving..." : <>Save <ArrowRight size={14} /></>}
                  </button>
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="lg:w-1/3">
              <div
                ...
                className="relative group/preview rounded-3xl aspect-[4/3] overflow-hidden bg-gray-50 border-2 border-primary-100 transition-all shadow-inner cursor-pointer"
              >
                {editForm.image_url ? (
                  <>
                  ...
                  <div className="w-full h-full flex flex-col items-center justify-center text-slate-300 gap-2">
                    {isCropping ? (
                      ...
                      <>
                        ...
                        <span className="text-[9px] font-black uppercase tracking-[0.2em] opacity-50 text-center px-4">Click to Replace Media</span>
                      </>
                    )}
                  </div>
                )}
            ...
            <div className="flex-1 space-y-3">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5 block">Capture Location</label>
                  <div className="relative">
                    <MapPin className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-300" size={16} />
                    <select
                ...
                <div>
                  <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5 block">Group Profile</label>
                  <div className="relative">
                    <UsersIcon className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-300" size={16} />
                    <select
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-slate-100">
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2 cursor-pointer group/toggle" onClick={() => setEditForm({ ...editForm, is_featured: !editForm.is_featured })}>
                    <div className={`w-8 h-5 rounded-full transition-all duration-300 flex items-center px-1 ${editForm.is_featured ? 'bg-amber-400 shadow-lg shadow-amber-400/20' : 'bg-slate-300'}`}>
                      <div className={`w-3 h-3 rounded-full bg-white transition-all duration-300 ${editForm.is_featured ? 'translate-x-3' : 'translate-x-0'}`} />
                    </div>
                    <span className={`text-[9px] font-black uppercase tracking-widest ${editForm.is_featured ? 'text-amber-600' : 'text-slate-400'}`}>Featured</span>
                  </div>
                  <div className="flex items-center gap-2 cursor-pointer group/toggle" onClick={() => setEditForm({ ...editForm, is_active: !editForm.is_active })}>
                    <div className={`w-8 h-5 rounded-full transition-all duration-300 flex items-center px-1 ${editForm.is_active ? 'bg-emerald-500 shadow-lg shadow-emerald-500/20' : 'bg-slate-300'}`}>
                      <div className={`w-3 h-3 rounded-full bg-white transition-all duration-300 ${editForm.is_active ? 'translate-x-3' : 'translate-x-0'}`} />
                    </div>
                    <span className={`text-[9px] font-black uppercase tracking-widest ${editForm.is_active ? 'text-emerald-600' : 'text-slate-400'}`}>Visible</span>
                  </div>
                </div>
                ...
                <div className="flex gap-3 w-full sm:w-auto">
                  ...
                  <button
                    ...
                    className="flex-1 sm:flex-none px-6 py-3 rounded-xl bg-primary-600 text-white text-[10px] font-black uppercase tracking-widest hover:bg-primary-700 shadow-xl shadow-primary-500/20 active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                  >
                    {isActionLoading ? "Syncing..." : <>Apply <ArrowRight size={14} /></>}
                  </button>
                </div>
              </div>
            </div>
